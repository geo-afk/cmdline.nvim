...
M.available = true -- Assume native Tree-sitter is available in Neovim

function M.setup()
	-- Ensure required parsers are installed/available
	M.ensure_parsers()
end

---Ensure required parsers are available
function M.ensure_parsers()
	local required = { "vim", "lua", "bash", "regex" }
	for _, lang in ipairs(required) do
		local res = vim.treesitter.language.add(lang)
		if type(res) == "string" then
			vim.notify(string.format("Tree-sitter parser for %s not available: %s", lang, res), vim.log.levels.WARN)
		end
	end
end
...
---@param source string
---@return table[] tokens
function M.extract_tokens(ast, source)
	...
	for id, node, _ in query:iter_captures(ast.root, source, 0, -1) do
...
function M.fallback_highlights(text, mode)
	...
	if mode == ":" then
		...
		-- Highlight strings
		for start_pos, _, end_pos in text:gmatch("()([\"'])(.-)%2()") do
			table.insert(highlights, {
				group = "String",
				line = 0,
